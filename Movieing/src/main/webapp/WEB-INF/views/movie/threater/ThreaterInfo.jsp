   ...
   $(document).ready(function(){
      $('.btn').on('click',function(){
         ...
         if($(this).hasClass('btn-outline-secondary')){//지역버튼 누를경우
            ...
            $(this).html('<i class="fas fa-check"></i>'+selText);
         }else {//"가산"이 눌린 경우
            console.log('영화관 버튼');
            var selText=$(this).text();
            $(this).html('<i class="fas fa-check"></i>'+selText).css('color','white');
            
            //지도 부착
			$('#mapDiv').html('<div id="map" style="width:100%;height:350px;"></div>');  
			makingMap();//지도 모양 채워줘
			...
			$.ajax({
				...
				data:
	              ...
	              {   /*데이터를 전송하기 전에 헤더에 csrf값을 설정한다*/
					...
					success:function(movieMap){//서버로 부터 정상적인 응답을 받았을 때(200번)
						   function replaceAll(str, searchStr, replaceStr) {
							   return str.split(searchStr).join(replaceStr);
							 }
						console.log('아작스 성공ㅇㅣ니'+movieMap);
						var timeTableString = '<h3>상영시간표</h3>';
						console.log('이거map되냐',movieMap['movieList']);
						$.each(movieMap['movieList'],function(index_1,m){
							timeTableString += '<div class="card ">'+
													'<div class=" card-header">'+
														'<span style="font-size: 2em;font-weight: bold">'+replaceAll(decodeURIComponent(m['movieTitle']),'+',' ')+'</span>'+
														'<button type="button" class="btn btn-grey">'+
															'<i class="fas fa-angle-right"></i>'+
														'</button>'+
													 '</div>'+
													 '<div class="card-body">';
													 $.each(m['dimensionList'],function(index_2,d){
														 timeTableString += '<div class="row px-3">'+
														 						'<p style="font-weight: bold;">'+replaceAll(decodeURIComponent(d['dimension']),'+',' ')+'</p>'+
																			 '</div>'+
																			 '<div class="row" style="text-align: center;">';
																
																$.each(d['timeList'],function(index_3,t){
																	 timeTableString += '<div class="card" style="width:150px">'+
																							 '<ul class="list-group">'+
																								 '<li class="list-group-item">'+decodeURIComponent(t['seat'])+'</li>'+
																								 '<li class="list-group-item">'+t['time']+'</li>'+
																								 '<li class="list-group-item">'+replaceAll(decodeURIComponent(t['seatCount']),'+','')+'</li>'+
																							  '</ul>'+
																						'</div>';
																 });
														 timeTableString += '</div>';
						...
						console.log('나는 타임스트링',timeTableString);
						$('#movieTimeTable').html(timeTableString);
					},	
